...
export async function POST(request: Request) {
  return withIdempotency(request, async () => {
    try {
      ...
      const { data: group, error: groupError } = await adaptedWrite(
        async () => service
          .from("groups")
          .insert({
            name,
        ...
        async (result) => {
          ...
          await emitCrdtOperation({
            scopeKey: groupScopeKey(result.data.id),
            documentType: "groups",
            entityType: "group",
          ...
          await emitCrdtOperation({
            ...
            entityId: result.data.id,
            action: "create",
            payload: { group_id: result.data.id, role: "admin" },
          });
        },
      );
      ...
      const { error: memberError } = await adaptedWrite(
        async () => service
          .from("group_members")
          .insert({ group_id: group.id, user_id: user.id, role: "admin" }),
        async () => {
          await emitCrdtOperation({
            scopeKey: groupScopeKey(group.id),
            documentType: "group_members",
            entityType: "group_member",
            entityId: `${group.id}:${user.id}`,
            action: "create",
            payload: { group_id: group.id, user_id: user.id, role: "admin" },
          });
        },
      );
