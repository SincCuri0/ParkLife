...
export async function PATCH(
  ...
  return withIdempotency(request, async () => {
    try {
        ...
        const { error: memberError } = await adaptedWrite(
          ...
          async () => {
            await emitCrdtOperation({
              ...
              entityId: `${id}:${joinRequest.user_id}`,
              action: "create",
              payload: { group_id: id, user_id: joinRequest.user_id, role: "member" },
            });
            await emitCrdtOperation({
              scopeKey: userScopeKey(joinRequest.user_id),
              documentType: "group_members",
              entityType: "group_member",
              entityId: `${id}:${joinRequest.user_id}`,
              action: "create",
      ...
      const { error: statusError } = await adaptedWrite(
        async () => service
          .from("join_requests")
